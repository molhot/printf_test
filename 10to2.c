...
char *basenum_char_malloc(int subject_num, int base_num)
{
	int malloc_size;
	int subjectnum_copy;
	char *char_malloc;
	...
	*(char_malloc + malloc_size) = '\0';
	while(malloc_size-- != 0)
	{
		*(char_malloc + malloc_size) = subjectnum_copy % base_num + '0';
...
int main()
{
	...
	printf("%d\n", base_num);
	char_subject_num = basenum_char_malloc(subject_num, base_num);
	printf("%s\n", char_subject_num);
